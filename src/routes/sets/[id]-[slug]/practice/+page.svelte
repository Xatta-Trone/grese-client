...
<Modal title="Settings" bind:open={clickOutsideModal} autoclose outsideclose>
  ...
  <P
    >Total Questions: <Input
      type="number"
      max={backupWords.length}
      min={2}
      bind:value={questionRemaining}
      size="sm"
      placeholder="Total questions...."
      on:change={(event) => {
        const val = parseInt(event?.target?.value ?? 0);

        if (val > backupWords.length) {
          questionRemaining = backupWords.length;
          totalQuestion = backupWords.length;
        } else {
          questionRemaining = val;
          totalQuestion = val;
        }
        if (val <= 2) {
          questionRemaining = 2;
